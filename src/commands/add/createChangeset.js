...
import getDependencyGraph from "../../utils/fromBolt/getDependencyGraph";
...
async function getPackagesToRelease(changedPackages, allPackages) {
  ...
  if (packagesToRelease.length === 0) {
    do {
      console.error("You must select at least one package to release");
      console.error("(You most likely hit enter instead of space!)");
...
async function createChangeset(changedPackages, opts = {}) {
  ...
  const dependencyGraph = await getDependencyGraph(cwd);
  ...
  while (summary.length === 0) {
    console.error("A summary is required for the changelog! 😪");
