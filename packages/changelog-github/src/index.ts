...
function readEnv(options: null | Record<string, any>) {
...
const changelogFunctions: ChangelogFunctions = {
  ...
  getReleaseLine: async (changeset, type, options) => {
    const { GITHUB_SERVER_URL, GITHUB_REPOSITORY } = readEnv(options);
    ...
    const links = await (async () => {
      if (prFromSummary !== undefined) {
        ...
        if (commitFromSummary) {
          ...
          links = {
            ...links,
            commit: `[\`${shortCommitId}\`](${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}/commit/${commitFromSummary})`,
          };
        }
        return links;
      }
