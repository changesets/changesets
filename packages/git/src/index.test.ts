...
describe("git - getDivergedCommit", () => {
...
describe("git - tag", () => {
  it("should create a tag for the current head", async () => {
    ...
    const tagRef = await spawn("git", ["rev-list", "-n", "1", "tag_message"], {
      cwd,
    });
...
describe("git - getCommitsThatAddFiles", () => {
...
describe("git - getCommitsThatAddFiles - with shallow clone", () => {
  ...
  it("reads the SHA of a file-add even if not already included in the shallow clone", async () => {
    ...
    const commit = (await getCommitsThatAddFiles(["b.js"], { cwd: clone }))[0];
  ...
  it("reads the SHA of a file-add even if the first commit of a repo", async () => {
    ...
    const commit = (await getCommitsThatAddFiles(["a.js"], { cwd: clone }))[0];
    expect(commit).toEqual(originalCommit);
...
describe("git - getChangedFilesSince", () => {
...
describe("git - getChangedPackagesSinceRef", () => {
