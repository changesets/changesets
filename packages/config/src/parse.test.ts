...
describe("validateConfig", () => {
  ...
  describe("rules", () => {
    ...
    describe("internalButNotGitHubRegistry", () => {
      it("should warn if access is internal but publishConfig.registry is not GitHub Packages Registry", async () => {
        const cwd = await testdir({
          "package.json": JSON.stringify(rootManifest),
        });
      ...
      it("should warn if access is internal but publishConfig.registry is not GitHub Packages Registry in a monorepo", async () => {
        const pkgA = {
          name: "pkg-a",
          private: true,
          version: "1.0.0",
      ...
      it("does not warn if all packages have registry configured", async () => {
        const pkgA = {
          name: "pkg-a",
          private: true,
          version: "1.0.0",
