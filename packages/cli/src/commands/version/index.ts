...
export default async function version(
  cwd: string,
  config: Config,
  snapshotReleaseConfig?: SnapshotReleaseConfig
) {
  ...
  if (
    snapshotReleaseConfig === undefined &&
    preState !== undefined &&
    preState.mode === "pre"
  ) {
  ...
  await applyReleasePlan(releasePlan, packages, config, snapshotReleaseConfig);

  if (snapshotReleaseConfig === undefined) return;
