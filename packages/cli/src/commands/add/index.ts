...
export const addCommand = defineWithContext({
  entry: true,
  name: "add",
  description: "Create a new changeset (default)",
  examples: "changeset add --open",
  args: {
    empty: {
      type: "boolean",
      description: "Skip prompting for a changeset summary",
    },
    message: {
      type: "string",
      alias: "m",
    ...
    open: {
      type: "boolean",
      description: "Automatically open the changeset in your editor",
      conflicts: ["empty"],
    },
    since: {
      type: "string",
      alias: "s",
      description:
        "Git ref to compare against when determining which packages have changed",
    },
  },
  run: async (ctx) => {
    const { cwd = process.cwd() } = ctx.env;
    const { empty, message, open, since } = ctx.values;
    const { packages, config } = ctx.extensions;
