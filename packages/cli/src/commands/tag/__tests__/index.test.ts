...
describe("tag command", () => {
  ...
  it("workspace project - tags all packages", async () => {
  ...
  it("workspace project - skips tags that already exist", async () => {
    const cwd = await testdir({
      "package.json": JSON.stringify({
        private: true,
        workspaces: ["packages/*"],
  ...
  it("single package repo - uses a simplified version-only tag", async () => {
    const cwd = await testdir({
      "package.json": JSON.stringify({
        private: true,
        name: "root-only",
  ...
  it("single package repo - does not tag on private", async () => {
    const cwd = await testdir({
      "package.json": JSON.stringify({
        private: true,
        name: "root-only",
