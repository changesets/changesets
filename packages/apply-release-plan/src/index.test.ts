...
describe("apply release plan - versioning formatting", () => {
...
describe("apply release plan - versioning", () => {
  ...
  it("should not bump peerDependencies if they are still in range when onlyUpdatePeerDependentsWhenOutOfRange set to true", async () => {
    let { changedFiles } = await testSetup(
      {
        "package.json": JSON.stringify({
          private: true,
...
describe("apply release plan - internal dependency bumping - patch", () => {
...
describe("apply release plan - internal dependency bumping - minor", () => {
...
describe("apply release plan - changelogs", () => {
  ...
  it("should add changelog an updated dependencies line when dependencies have been updated", async () => {
    let { changedFiles } = await testSetup(
      {
        "package.json": JSON.stringify({
          private: true,
...
describe("apply release plan - changesets", () => {
...
describe("apply release plan - errors", () => {
  ...
  it.skip("should error and not write ifa package appears twice", async () => {
    expect(async () => {
      await testSetup(
        {
          "package.json": JSON.stringify({
  ...
  it("should error and not write if a package cannot be found", async () => {
  ...
  it(
    "should error and not write if a provided changelog function fails",
    temporarilySilenceLogs(async () => {
      ...
      const consoleErrorMock = console.error as jest.Mock<typeof console.error>;
...
`);
  ...
  it("shouldn't commit updated files from packages", async () => {
    ...
    expect(gitCmd.stdout.toString()).toContain("Changes not staged for commit");
  ...
  it("should remove applied changesets", async () => {
    ...
    const changesetsDeleted = releasePlan.changesets.reduce((prev, { id }) => {
      return (
        prev &&
