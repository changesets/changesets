...
Several of these have associated type definitions, which you can find in [our types package](https://github.com/changesets/changesets/blob/main/packages/types).
...
- **fixed packages** - Fixed packages share a semver categorisation, such that all fixed packages have the same semver version and are always published together. The logistics of this are best left to our [fixed-packages](../advanced/fixed-packages.md) documentation.
- **linked packages** - Linked packages share a semver categorisation, such that all published linked packages have consistent new semver ranges. The logistics of this are best left to our [linked-packages](../advanced/linked-packages.md) documentation.
